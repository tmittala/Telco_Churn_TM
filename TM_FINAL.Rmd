...
# QUESTION 2 - K-Means Cluster Model
...
# QUESTION 3 - OUTLIERS IN 'MONTHLY CHARGES' COLUMN
For our univariate outlier analysis, let's check the summary of the "monthly charges" first. We will use the raw data to do outlier analysis.

## Data and Visual Exploration
```{r}
summary(telco_raw$MonthlyCharges)
```
### Histogram of Monthly Charges
```{r}
mc_hist <- ggplot(telco_raw) + aes(x = MonthlyCharges)+ geom_histogram(bins = 30, fill = "royalblue") + theme_minimal()
mc_hist
```

The histogram shows that monthly charges are skewed to the right which is expected as monthly charges would tend to be low for most customers. However, we don't see any specific values or patterns that may suggest presence of outlier.

### Box Plot of Monthly Charges
Utilizing a boxplot visualization allows for a quick and effective examination of potential outliers, represented as points beyond the whiskers of the plot. This analysis serves as a valuable step in uncovering irregularities in monthly charges, contributing to a comprehensive understanding of the data in general.

```{r}
mc_box <- ggplot(telco_raw) + aes(x= "", y = MonthlyCharges) + geom_boxplot(fill = "lightblue") + theme_minimal()
mc_box
```

The box plot reveals no outliers in the monthly changes column. Let's confirm it using the IQR and the 'boxplot.stats' function. Here, any value not between $(q_0 - 1.5 IQR)$ and $(q_0.75 + 1.5 IQR)$ will be considered out of normal range.

```{r}
boxplot.stats(telco_raw$MonthlyCharges)$out
```

Upon executing the boxplot.stats function on the Telco Churn dataset's Monthly Charges column, no outliers were detected, indicating a relatively uniform distribution of monthly charges without extreme values.

## Hempel Filter using MAD

Implementing the Hempel Filter using the Median Absolute Deviation (MAD) for outlier detection in our dataset provides a robust method to identify and address potential anomalies. By calculating the MAD and establishing a threshold, we can systematically filter out outliers, ensuring a more accurate and resilient analysis of our data. This approach enhances the reliability of our findings by minimizing the impact of extreme values on the overall dataset.

**We will consider data points as outliers if they are outside the 6-MAD range around the median of the data.**

```{r}
lower_mad_bound <- median(telco_raw$MonthlyCharges) - 3*mad(telco_raw$MonthlyCharges, constant = 1)
upper_mad_bound <- median(telco_raw$MonthlyCharges) + 3*mad(telco_raw$MonthlyCharges, constant = 1)

outlier_ind <- which(telco_raw$MonthlyCharges < lower_mad_bound | telco_raw$MonthlyCharges > upper_mad_bound)
outlier_ind
```

Upon applying the Hempel Filter using the Median Absolute Deviation (MAD) to our dataset, no outliers were detected, affirming the robustness of this approach in maintaining data integrity. This result suggests a dataset with a consistent distribution, contributing to the reliability of our analysis.

## Outlier Analysis
Upon careful analysis, it is noteworthy that **no outliers were detected in the Monthly Charges variable**. The absence of outliers suggests a relatively homogenous distribution of monthly charges among Telco customers. This finding indicates a more consistent pattern in the pricing structure and a lack of extreme values that could potentially skew our statistical analyses.
